package au.com.chris.antdisplay.adapters;
...
public class MultiItemListRowAdapter implements WrapperListAdapter {
	...
	public MultiItemListRowAdapter(Context context, ListAdapter adapter,
			int itemsPerRow, int cellSpacing) {
		if (itemsPerRow <= 0) {
			throw new IllegalArgumentException(
					"Number of items per row must be positive");
		}
		mContextReference = new WeakReference<Context>(context);
		mAdapter = adapter;
		mItemsPerRow = itemsPerRow;
		mCellSpacing = cellSpacing;
		mItemLayoutParams = new LinearLayout.LayoutParams(0,
				LayoutParams.MATCH_PARENT);
		mItemLayoutParams.setMargins(cellSpacing, cellSpacing, 0, 0);
		mItemLayoutParams.weight = 1;
		mRowLayoutParams = new AbsListView.LayoutParams(
